...
//PRE=(A contiene una sequenza di interi che termina con -2 e che consiste di una serie (possibilmente vuota) di sottosequenze ciascuna delle quali terminacon -1, inoltre n >=0)
int S(int* A, int n)
{
	int count = 0;
	int posI=0;

	for(int j=0; (A[j] != -2) && (count < n); j++)
	{
		if (A[j] == -1 && A[j+1] != -2)
		{
			posI = j+1;
